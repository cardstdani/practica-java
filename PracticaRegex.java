...
class PracticaRegex {
    ...
    public static LinkedTreeMap<String, LinkedTreeMap<String, String>> map = new LinkedTreeMap<>();

    public static void main(String[] args) {
        Scanner in = new Scanner(System.in);
        try {
            JsonElement json = JsonParser.parseReader(new InputStreamReader(new URL("https://github.com/cardstdani/practica-java/blob/main/Data/patterns_data.txt?raw=true").openStream(), "UTF-8"));
            Gson gson = new Gson();
            map = (LinkedTreeMap<String, LinkedTreeMap<String, String>>) gson.fromJson(json, map.getClass());
        } catch (Exception e) {
            System.out.println("Error cargando patrones");
        }
        ...
        do {
            ...
            for (int intento = 0; (intento < intentos) & !Arrays.equals(entrada, new int[]{2, 2, 2, 2, 2}); intento++) {
                ...
                if (validar(entrada, word, globalPattern)) {
                    String pattern = generatePattern(Arrays.stream(entrada).mapToObj(String::valueOf).collect(Collectors.joining("")), word);
    ...
    public static String generatePattern(String entrada, String word) {
        return map.get(word).get(entrada);
    }
    ...
    public static LinkedHashMap<String, Double> updateDict(LinkedHashMap<String, Double> diccionario, String pattern) {
        ...
        for (RunnableFuture<LinkedHashMap<String, Double>> t : threads) {
            try {
                ...
                System.out.println("Error en threads");
                e.printStackTrace();
            }
        }
        return tmpDict;
